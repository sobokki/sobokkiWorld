...
const {register, handleSubmit,setValue,formState: { errors },} = useForm<HookFormTypes>();
...
<Input type="text" id="nickname" label="닉네임"  placeholder="당신의 닉네임을 적어주세요" 
value={nickname}
...
<div className={styles.formArea}>
    ...
    {...register("title", { required: "제목은 필수 입력입니다.", maxLength: { value: 5, message: "5글자로 써주셔야합니다." }, minLength: { value: 5, message: "5글자로 써주셔야합니다." } })}
...
<textarea  id="reason" placeholder="그렇게 생각한 이유는?"  className={errors.reason? styles.errorReason: styles.reason} 
{...register("reason",{required:"이유는 필수 입력입니다."})}/>
<small>{errors.reason?.message}</small>
</div>

<div className={styles.buttons}>
<button className={styles.registration} type="submit">등록</button>
